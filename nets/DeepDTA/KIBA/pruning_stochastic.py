...
import pickle
import os
from compressionNN import pruning_stochastic
from keras import optimizers
import numpy as np
import sys
...
c = [int(np.log2(int(argument_list[i]))) for i in range (3,len(argument_list))]
c_print = [int(argument_list[i]) for i in range (3,len(argument_list))]
...
pr_ws_model = pruning_stochastic.PruningStochastic_NN(model=model, perc_prun_for_dense=p, bits_for_dense_layers=c,index_first_dense=index, apply_compression_bias=b, div=div)

pre_ws_train = pr_ws_model.model.evaluate(x_train, y_train)
pre_ws_test = pr_ws_model.model.evaluate(x_test, y_test)

print("load weights in ws model, train acc -->", pre_ws_train)
print("load weights in ws model, test acc -->", pre_ws_test)

pr_ws_model.apply_pruning_stochastic()

pr_ws_model.set_loss(tf.keras.losses.MeanSquaredError())
pr_ws_model.set_optimizer(tf.keras.optimizers.Adam(learning_rate=0.0005)) 

post_ws_train = pr_ws_model.model.evaluate(x_train, y_train)
post_ws_test = pr_ws_model.model.evaluate(x_test, y_test)
...
pr_ws_model.train_ws_deepdta(epochs=100, lr=l, dataset=dataset, X_train=x_train, y_train=y_train, X_test=x_test, y_test=y_test, patience=0)

TRAIN_RES = ([pre_ws_train] + [post_ws_train] + pr_ws_model.acc_train)
TEST_RES = ([pre_ws_test] + [post_ws_test] + pr_ws_model.acc_test)

TRAIN_RES = ([post_ws_train] + pr_ws_model.acc_train)
TEST_RES = ([post_ws_test] + pr_ws_model.acc_test)

with open("deepdta_kiba_pr_st.txt", "a+") as tex:
    tex.write("{} lr {} cluster {} -->\n {}\n , {}\n\n".format(b,l, c_print, TRAIN_RES, TEST_RES))

DIR="pruning_stochastic"
TO_SAVE = "{}-{}-{}".format(p,c_print,round(TEST_RES[-1],5))
