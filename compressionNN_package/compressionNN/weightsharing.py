...
class Weightsharing_NN(compressed_nn.Compressed_NN):
    def __init__(self, model, clusters_for_dense_layers, index_first_dense, apply_compression_bias=False, div=None):
        ...
        #self.list_weights = model.get_weights()
        if div:
            self.div=div
        else:
    ...
    def update_centers_and_recompose(self, list_weights_before, lr):
        ...
        d = find_index_first_dense(list_weights)
        centers_upd = [(centroid_gradient_matrix(self.idx_layers[(i-d)//div], list_weights_before[i]-list_weights[i], self.clusters[(i-d)//div])) for i in range(d,len(list_weights), div)]
    ...
    #@tf.function
    def train_ws(self, epochs, lr, dataset, X_train, y_train, X_test, y_test, step_per_epoch=None, patience=-1):
