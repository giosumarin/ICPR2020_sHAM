...
class PruningWeightsharing_NN(pruning.Pruning_NN, weightsharing.Weightsharing_NN):
    ...
    def update_centers_and_recompose_comb(self, list_weights_before, lr):
        ...
        masks_for_centers = [self.masks[i] for i in range(0, len(self.masks), self.div)]
        # for m in masks_for_centers:
        #     a,b = np.unique(m, return_counts=True)
        #     print(a,b)
        centers_upd = [(centroid_gradient_matrix_combined(self.idx_layers[(i-d)//div], list_weights_before[i]-list_weights[i], self.clusters[(i-d)//div], masks_for_centers[(i-d)//div])) for i in range(d,len(list_weights), div)]
